                            ...
                            <tr class="odd gradeX">
                                <td>
                                    <a class="media-left" href="javascript:;">
                                        <img src="{{ (!empty($user->profile_pic)) ? Storage::disk('local')->url("avatars/$user->profile_pic") : '/assets/img/avatars/m-silhouette.jpg' }}"
                                             class="rounded-circle " width="36" height="36" alt="">
                                    </a>
                                    <div class="media-body">
                                        <h4  class="media-heading"> &ensp; {{ (!empty($user->first_name)) ?  $user->first_name : ''}}</h4>
                                        <p> &ensp; &ensp;{{ (!empty($user->positions)) ?  $user->positions : ''}} </p>
                                    </div>
                                </td>
                                <td>{{ (!empty($user->profile_pic)) ?  $user->employee_number : ''}} </td>
                                <td>{{ (!empty($user->email)) ?  $user->email : ''}} </td>
                                <td>{{ (!empty($user->cell_number)) ?  $user->cell_number : ''}} </td>
                                ...
                                @endif
                                <td>{{ (!empty($user->date_joined)) ? date(' d M Y', $user->date_joined) :'' }} </td>
                                <td>
                                    <a href="{{ '/users/' . $user->hashid }}" class="btn btn-success  btn-sm"
                                       data-id="{{ $user->hashid }}"> View Details <i class="fa fa-arrow-right"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
